...
(function () {
	...
	function injectStyles(styleHandles) {
		...
		var validHandles = styleHandles.filter(
			function (handle) {
	...
	function injectScripts(scriptAssets) {
		...
		var validAssets = scriptAssets.filter(
			function (asset) {
				...
				if (gallery) {
					...
					if (galleryDataAttr) {
						try {
							...
							// CRITICAL: Identify clicked image by ID (preferred) or by matching image src URL.
							// When randomOrder is enabled, WordPress Core shuffles the HTML display order,
							// but the galleryData.images array is reordered by PHP to match the HTML display order.
							// This ensures clicking an image opens the correct image in the modal.
							// See: class-sppopups-gallery.php::fix_random_order_gallery_data() for PHP-side fix.
							var clickedImageId    = galleryItem.getAttribute( 'data-image-id' );
							var clickedImageIndex = -1;

							// Try to find by ID first (most reliable - works regardless of display order).
							if (clickedImageId) {
		...
		window.sppopupsGallery.init(
			{
				...
				setCurrentMaxWidth: function (width) {
