...
class SPPopups_Settings {
	...
	public function render_tldr_section_description() {
		...
		?>
		<p><?php esc_html_e( 'Configure the AI-powered TLDR feature that generates page summaries on-demand.', 'sppopups' ); ?></p>
		<?php if ( ! $ai_available['plugin_active'] ) : ?>
			<div class="notice notice-warning inline">
				<p><?php esc_html_e( 'AI Experiments plugin is not active. TLDR feature requires the AI Experiments plugin to be installed and activated.', 'sppopups' ); ?></p>
			</div>
		<?php elseif ( ! $ai_available['credentials'] ) : ?>
			<div class="notice notice-warning inline">
				<p><?php esc_html_e( 'AI credentials are not configured. Please configure AI credentials in Settings → AI Experiments.', 'sppopups' ); ?></p>
			</div>
		<?php else : ?>
			<div class="notice notice-success inline">
				<p><?php esc_html_e( 'AI Experiments plugin is active and credentials are configured.', 'sppopups' ); ?></p>
			</div>
		<?php endif; ?>
		<?php
	}
	...
	public function render_tldr_enabled_field() {
		...
		<label>
			...
			<?php esc_html_e( 'Enable AI-powered TLDR feature', 'sppopups' ); ?>
		</label>
		<p class="description">
			<?php esc_html_e( 'When enabled, users can click elements with class "spp-trigger-tldr" to generate and display AI-powered summaries of the current page.', 'sppopups' ); ?>
		</p>
		<?php
	}
	...
	public function render_tldr_prompt_field() {
		...
		<p class="description">
			<?php esc_html_e( 'The prompt template used to generate TLDR summaries. Use {content} as a placeholder for the page content.', 'sppopups' ); ?>
		</p>
		<?php
	}
	...
	public function render_tldr_cache_ttl_field() {
		...
		<p class="description">
			<?php esc_html_e( 'How long to cache generated TLDR summaries (in hours). Default: 12 hours.', 'sppopups' ); ?>
		</p>
		<?php
	}
	...
	public function render_settings_section() {
		...
		<div class="sppopups-settings-section" style="margin-top: 40px; padding: 24px; background: #f6f7f7; border: 1px solid #dcdcde; border-radius: 4px; max-width: 800px;">
			<h2 style="margin-top: 0; margin-bottom: 16px; font-size: 18px; font-weight: 600; color: #1d2327;">
				<?php esc_html_e( 'AI TLDR Settings', 'sppopups' ); ?>
			</h2>
			...
			<form method="post" action="">
						...
						<tr>
							<th scope="row">
								<label for="sppopups_tldr_enabled">
									<?php esc_html_e( 'Enable TLDR Feature', 'sppopups' ); ?>
								</label>
							</th>
							<td>
								<label>
						...
						<tr>
							<th scope="row">
								<label for="sppopups_tldr_prompt">
									<?php esc_html_e( 'TLDR Prompt Template', 'sppopups' ); ?>
								</label>
							</th>
							<td>
						...
						<tr>
							<th scope="row">
								<label for="sppopups_tldr_cache_ttl">
									<?php esc_html_e( 'Cache Duration', 'sppopups' ); ?>
								</label>
							</th>
							<td>
								...
								<span style="margin-left: 8px;"><?php esc_html_e( 'hours', 'sppopups' ); ?></span>
								<p class="description" style="margin-top: 8px;">
									<?php esc_html_e( 'How long to cache generated TLDR summaries. Default: 12 hours.', 'sppopups' ); ?>
								</p>
							</td>
						</tr>
					</tbody>
				...
				<?php submit_button( __( 'Save TLDR Settings', 'sppopups' ), 'primary', 'save_tldr_settings', false ); ?>
			</form>
		</div>
		<?php
	}
