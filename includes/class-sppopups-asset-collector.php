...
/**
 * SPPopups_Asset_Collector class.
 *
 * @package SPPopups
 */
class SPPopups_Asset_Collector {
	...
	public function get_style_handles() {
		...
		// Note: Don't add it here - let it be handled by the asset data collection.
		...
		$styles = array_filter(
			$styles,
			function ( $handle ) use ( $editor_prefixes ) {
	 ...
	 *
	 * @param string $_handle Style handle.
	 * @return bool True if style should be considered loaded
	 */
	public function is_style_loaded( $_handle ) {
	...
	public function get_asset_data() {
		...
		$style_handles = array_filter(
			$style_handles,
			function ( $handle ) use ( $is_editor_style ) {
